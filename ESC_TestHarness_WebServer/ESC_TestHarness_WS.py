        ...
        try:
          ...
          if (JSON_Request_Data['Status'] == "Done") :
                    ...
                    if received in T_Start.keys():
                        ...
                        app.logger.info('LV Response: Test_File: %s, Start_T: %d, End_T: %d', received, T_Start[received], T_End[received], '/n')
...
def Input_From_File():
   ...
   #sanity check 
    ...
    for line in My_Input_File: 
        ...
        print "\n"; app.logger.info('Instruction sent: TEST FILE: %s, %s, %s', data["Test_File"], data['RF_Center_Freq'], data['RF_Gain']); print "\n"
...
def Execute_Tests():
    ...
    for item in Test_Dir_Files:
        ...
        c = pycurl.Curl()
        c.setopt(c.HTTPHEADER, ['Accept: application/json', 'Content-Type: application/json', 'charsets: utf-8'])
        # c.setopt(c.URL, 'http://' + LV + ':8001/v1/ESC_Harness_InstructionsListener') 
        c.setopt(c.URL, 'http://' + LV + ':8001/v1/Main_System') 
        #c.setopt(c.URL, 'http://' + LV + ':5000') 
        # send all data to this function
        c.setopt(c.WRITEFUNCTION, buf.write)
        # some servers don't like requests that are made without a user-agent field, so we provide one
        c.setopt(c.USERAGENT, 'libcurl-agent/1.0')
...
if __name__ == '__main__':
    ...
    #os.rename(".", "")
    fh = logging.handlers.RotatingFileHandler('ESC-Test_'+datetime.datetime.now().strftime("%Y%m%d-%H%M%S")+".log", maxBytes=10000000, backupCount=5)
